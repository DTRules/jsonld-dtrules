...
//#Decision Table Compiler that produces a DT function with tracing
//
//
//#Code
...
//**trace** - returns an anonymous function that implements the DT; 
...
function trace(dt) {
    ...
    function genDtSrc(dt) {
        ...
        function forLoopsSrc() {
            ...
            if (!dt[dt_p.for_loops]) {
                return traceDtNameSrc() + funcStaticVarSrc + funcStaticSrc;
            }
            forLoops = dt[dt_p.for_loops]['@list'];
            for (i = 0; i < forLoops.length; i += 1) {
            ...
            return (indexVarSrc + traceDtNameSrc() + funcStaticVarSrc + src + funcStaticSrc + forWhereEndSrc + forLoopEndSrc);
    ...
    logParamCheckSrc = 'if (arguments.length !== mainDtFuncObj.parameterNames.length +1) {return \'missing log parameter\';} if (!Array.isArray(arguments[arguments.length-1])) {return \'the log parameter must be an array\';}';
    mainDTFuncBodySrc = '{' + logParamCheckSrc + dtModuleRequireSrc + innerDtVarSrc + mainDtFuncObj.funcBodySrc + '}';
    mainDtEvalSrc = 'mainDtFunc = function(' + parametersSrc(mainDtFuncObj.parameterNames) + ', log)' + mainDTFuncBodySrc;
